      ...
      <div className="row">
        ...
        <div className="col-lg-4">
          <form action="" onSubmit={(e) => { e.preventDefault() }}>
            ...
            <button type="submit" className="btn btn-primary mt-2" onClick={addItem}>AddList</button>
          </form>
        </div>
        <div className="col-lg-4"></div>
      </div>
          ...
          <ul type="none">
            {
              tasks && tasks.length > 0 ? tasks.map(({ task, id }) => (
                <li key={id}>{task} <span className="fs-2 text-danger" onClick={() => deleteItem(id)}><MdDelete /></span></li>
              )) : <li>No List Found</li>
            }
          </ul>
        </div>
